...
**Sample dev-sandbox upload:** <https://api06.dev.openstreetmap.org/changeset/616162> — one batch generated by the import tool against `master.apis.dev.openstreetmap.org`, evidence the upload mechanics work end-to-end.
**Live status:** tracked in the project README at <https://github.com/skfd/toronto-2-address-import#status>.

This proposal targets the [OSM Import Guidelines](https://wiki.openstreetmap.org/wiki/Import/Guidelines). All upload activity to date has used the OSM dev sandbox; no production edits will be made until this proposal has cleared the customary feedback window on `imports@openstreetmap.org` and the OSM wiki.
...
**Geographic scope:** the City of Toronto, Ontario — OSM admin boundary [relation 324211](https://www.openstreetmap.org/relation/324211), covering the post-1998 amalgamated city (former municipalities of Toronto, East York, Etobicoke, North York, Scarborough, and York). The 2,493 review tiles named in §3 partition this boundary.
...
- **OSM import account:** [`skfd imports`](https://www.openstreetmap.org/user/skfd%20imports) — a dedicated account created solely for this import, per the [OSM Import Guidelines](https://wiki.openstreetmap.org/wiki/Import/Guidelines) recommendation. The maintainer's personal OSM account (`skfd`) is not used for any upload from this tooling.
...
- **Phase 0 — community review.** Create the proposal page on the OSM wiki at `Toronto/Import/AddressPoints`, add a link to it from the `Import/Catalogue` index, post the announcement to `imports@openstreetmap.org`, and link the existing OSM Community Forum discussion thread (<https://community.openstreetmap.org/t/address-import-for-toronto/119368>) on the wiki page. Minimum two-week feedback window. Incorporate feedback, revise.
- **Phase 1 — pilot (1 tile).** Earliest start: no sooner than two weeks after the wiki page is published and the `imports@openstreetmap.org` post is sent (the customary feedback window). Tile `high-park-swansea-sw-se` — a depth-2 quadrant of the High Park-Swansea neighbourhood, bbox `(43.633436, -79.480592, 43.639157, -79.469502)`, 250 source addresses pre-conflation. End-to-end human review. All candidates manually approved even where auto-approval would normally apply. Post the changeset list to the wiki page after upload. Hold for one week for community response.
