...
def main(argv: list[str] | None = None) -> int:
    ...
    # Public upload manifest — one CSV per tile, plus a cumulative all.csv
    # the wiki page links to.
    all_rows: list[tuple[int, str, int, int]] = []
    for meta in per_run_meta:
        rid = meta["run_id"]
        rows = upload_manifest.fetch_for_run(rid)
        all_rows.extend(rows)
        label = meta["tile_id"] or f"run{rid}"
        upload_manifest.write_csv(out / "uploads" / f"{label}.csv", rows)
        copied.append(f"uploads/{label}.csv")
    all_rows.sort(key=lambda r: r[0])
    upload_manifest.write_csv(out / "uploads" / "all.csv", all_rows)
    copied.append(f"uploads/all.csv ({len(all_rows)} rows)")
